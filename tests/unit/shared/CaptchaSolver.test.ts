...
import { solveCaptcha } from "../../../src/shared/CaptchaSolver";
...
test("solveCaptcha should return false if captcha is detected", async () => {
  const page = await browser.newPage();
  await page.setContent('<html><iframe src="https://example.com/captcha"></iframe></html>');
  const result = await solveCaptcha(page);
...
test("solveCaptcha should handle errors gracefully", async () => {
  ...
  page.$ = jest.fn().mockRejectedValueOnce(new Error("Captcha detection failed"));
